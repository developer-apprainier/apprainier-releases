...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .apprainier-live-carousel{display:flex;flex-direction:column;touch-action:pan-y;user-select:none;-webkit-user-select:none;cursor:grab}.apprainier-live-carousel.dragging{cursor:grabbing}.apprainier-carousel-media{position:relative;flex:0 0 auto;overflow:hidden}.apprainier-carousel-track{height:100%;display:flex;will-change:transform;transition:transform 360ms cubic-bezier(.22,1,.36,1)}.apprainier-carousel-slide{height:100%;flex:0 0 100%;background-size:cover;background-position:center}.apprainier-carousel-arrow{position:absolute;top:50%;transform:translateY(-50%);z-index:2;width:42px;height:42px;border:0;border-radius:999px;background:rgba(15,23,42,.48);color:#fff;display:grid;place-items:center;font-size:30px;line-height:1;cursor:pointer;box-shadow:0 10px 24px rgba(15,23,42,.20);backdrop-filter:blur(10px);transition:transform .16s ease,background .16s ease,opacity .16s ease}.apprainier-carousel-arrow:hover{background:rgba(15,23,42,.68);transform:translateY(-50%) scale(1.04)}.apprainier-carousel-arrow:active{transform:translateY(-50%) scale(.96)}.apprainier-carousel-arrow.prev{left:14px}.apprainier-carousel-arrow.next{right:14px}.apprainier-carousel-copy{flex:1;background:var(--ar-live-bg,#fff);padding:16px 18px 14px;display:flex;flex-direction:column;justify-content:center;transition:opacity 180ms ease,transform 180ms ease}.apprainier-carousel-copy.changing{opacity:.55;transform:translateY(3px)}.apprainier-carousel-dots{display:flex;gap:7px;margin-top:10px}.apprainier-carousel-dots span{width:9px;height:9px;border-radius:999px;background:color-mix(in srgb,var(--ar-live-accent,#0f8f7f) 25%,transparent);transition:width .18s ease,background .18s ease;cursor:pointer}.apprainier-carousel-dots span.active{width:28px;background:var(--ar-live-accent,#0f8f7f)}
...
function parseLiveCardFontSize(value, fallback) {
  ...
  if (typeof value === 'string') {
    ...
    const numeric = Number(trimmed.replace(/px|sp|dp|rem|em/g, ''));
...
function renderCarouselLiveCard(root, card, structure, style, height, triggerId, options) {
  ...
  const copy = el('div', 'apprainier-carousel-copy');
  const title = el('div', 'apprainier-live-title');
  const subtitle = el('div', 'apprainier-live-subtitle');
  const dots = el('div', 'apprainier-carousel-dots');
  copy.append(title, subtitle, dots);
  media.append(track);
  root.append(media, copy);
  media.style.height = `${Math.min(Math.max(150, Math.floor(height * 0.74)), Math.floor(height))}px`;
  ...
  function paint() {
    ...
    window.setTimeout(() => copy.classList.remove('changing'), 120);
    title.textContent = item.title || card.title || card.name || 'Live Card';
    subtitle.textContent = item.subtitle || card.subtitle || '';
    title.style.color = resolveStyleColor(style.titleColor || style.textColor, '#0f172a');
    subtitle.style.color = resolveStyleColor(style.subtitleColor || style.textColor, '#475569');
    title.style.fontSize = `${parseLiveCardFontSize(item.titleSize || style.titleSize, 18)}px`;
    subtitle.style.fontSize = `${parseLiveCardFontSize(item.subtitleSize || style.subtitleSize || style.textSize, 15)}px`;
